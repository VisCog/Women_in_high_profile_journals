...
# Calculate SEM percentage female
percent_female_sem <- function(x) sd((x == "female") * 100, na.rm = TRUE) / sqrt(sum(!is.na(x)))
...
calculate_year_percent_female <- function(file) {
  ...
  # Calculate percentage female SEM authors by year
  sem_first <- with(journal, tapply(gender_first, year, percent_female_sem))
  sem_last  <- with(journal, tapply(gender_last, year, percent_female_sem))
  
  return(data.frame(year = as.numeric(names(female_first)), female_first, 
                    sem_first, female_last, sem_last, 
                    journal = gsub(".*/", "", file)))
}
