...
# Calculate population standard devation percentage female
percent_female_sd <- function(x) {
  x <- x[!is.na(x)] == "female"
...
calculate_year_percent_female <- function(file) {
  ...
  # Calculate percentage female SD authors by year
  sd_first <- with(journal, tapply(gender_first, year, percent_female_sd))
  sd_last  <- with(journal, tapply(gender_last, year, percent_female_sd))
